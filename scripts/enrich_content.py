#!/usr/bin/env python3
"""Enrich knowledge_content.py with high-quality Chinese content for all domains.

This script reads the existing knowledge_content.py, adds rich content entries
for all domains that currently only have generic fallback content, then writes
...
import knowledge_content as kc
existing = dict(kc.CONTENT)

# ── New rich content entries ──

NEW_CONTENT = {}

# ===== UI DOMAIN =====
NEW_CONTENT['ui'] = {}
NEW_CONTENT['ui']['ui/doc-1/'] = {
    'title': '组件库标准',
    'overview': '组件库是产品 UI 一致性的基石。统一的组件库能显著提升开发效率，降低设计差异带来的维护成本。本规范定义了组件的设计标准、命名规则、文件组织和使用约定，确保团队所有成员能够一致地构建和使用 UI 组件。',
    'principles': ['可复用：每个组件至少支持 3 种以上使用场景', '可组合：小组件通过组合形成复合组件，保持单一职责', '可定制：通过 props/parameters 暴露样式和行为的可控维度', '可测试：每个组件必须有 Storybook/Docs 用例和单元测试'],
    'rules': [
    ...
    'faqs': [
        ('组件库的粒度如何划分？', '原子组件（Button/Input）→ 分子组件（FormField/SearchBar）→ 有机体（DataTable/Form）→ 模板（PageLayout）。推荐原子和分子组件在组件库中维护，有机体和模板在业务项目中维护。'),
...
NEW_CONTENT['ui']['ui/doc-2/'] = {
    'title': '设计令牌系统',
    'overview': '设计令牌（Design Tokens）是设计和开发之间共享的视觉样式原子单位。通过令牌系统，我们可以确保产品在多平台（Web/iOS/Android）上保持一致的外观和体验。本规范定义了令牌的分类体系、命名规则和交付流程。',
    'principles': ['单一来源：所有视觉样式值统一在令牌中定义，禁止到处硬编码', '层级分明：全局令牌 → 别名令牌 → 组件令牌三级结构', '平台无关：令牌定义与平台无关，通过适配层转为各平台变量', '可追踪：每个令牌都有明确的用途文档和使用范围说明'],
    'rules': [
    ...
    'faqs': [
        ('令牌和变量的区别？', '令牌是设计侧的抽象概念（如 color-primary），变量是代码侧的实现方式（如 --color-primary: #1890ff）。令牌是跨平台的，变量是平台相关的。'),
...
NEW_CONTENT['ui']['ui/doc-3/'] = {
    'title': '响应式设计规范',
    'overview': '响应式设计确保应用在从手机到桌面大屏的各种设备上都能提供良好的用户体验。本规范定义了断点系统、布局网格、弹性组件和内容适配的标准做法。',
    'principles': ['移动优先：从小屏开始设计，逐步增强到大屏体验', '断点驱动：内容布局以断点为准，而非设备类型', '弹性组件：组件自身适应容器宽度，而非全局断点', '渐进增强：大屏提供更多信息和交互，不隐藏核心功能'],
    'rules': [
    ...
    'faqs': [
        ('应该适配所有设备还是主要设备？', '专注于断点区间而非特定设备。测试覆盖 xs(手机)、sm(大屏手机/小平板)、md(平板/小屏笔记本)、lg(桌面) 四个区间即可。'),
        ('响应式设计怎么影响性能？', '使用 CSS media queries 而非 JS 监听 resize。图片使用 srcset 按需加载。避免在移动端加载桌面端的大图资源。'),
    ],
    'checks': [
        '页面在 320px 宽度下可正常使用',
        '触摸目标 ≥ 44x44pt',
...
NEW_CONTENT['ui']['ui/doc-4/'] = {
    'title': '无障碍访问规范',
    'overview': '无障碍访问（A11Y）确保所有用户，包括残障人士，都能正常使用产品。本规范基于 WCAG 2.1 AA 标准，定义了颜色对比度、键盘导航、屏幕阅读器支持、焦点管理等方面的具体要求。',
    'principles': ['感知可辨：所有信息不能仅依赖单一感官（如颜色），需提供文本替代', '操作可健：所有功能必须可通过键盘操作', '理解可读：内容清晰可预测，输入辅助明确', '兼容可靠：最大兼容主流辅助技术（屏幕阅读器、语音控制等）'],
    'rules': [
    ...
    'faqs': [
        ('无障碍是必须的吗？', '在大多数行业（金融、政务、医疗）是合规要求。即使无强制要求，无障碍设计也能提升所有用户的体验。'),
...
NEW_CONTENT['ui']['ui/doc-5/'] = {
    ...
    'faqs': [
        ('深色模式下阴影怎么处理？', '深色模式的阴影应降低不透明度（从 0.15 降到 0.08），使用叠加层（overlay）而非阴影来表现层次。'),
        ('如何确保主题切换不闪烁？', '使用 CSS 变量方案（非 JS 类名切换），将主题色值存储在 CSS 变量中，切换 class 时自动更新。首屏主题在 HTML 元素上内联设置。'),
    ],
    'checks': [
        '浅色 + 深色模式完整定义',
...
NEW_CONTENT['ui']['ui/motion/'] = {
    'title': '动效设计规范',
    'overview': '界面动效提升用户体验的流畅感与品质感。本规范定义了动效的持续时间、缓动曲线、使用场景和实现标准，确保产品内的动效保持一致和协调。',
    'principles': ['有目的：每个动效必须服务于明确的交互目的（反馈/引导/过渡）', '不打扰：动效应快速（<300ms），不阻碍用户操作流程', '一致性：同类交互使用相同的动效参数（时长/曲线/位移）', '可关闭：尊重用户 prefers-reduced-motion 系统设置'],
    'rules': [
    ...
    'faqs': [
        ('所有元素都需要动效吗？', '不需要。优先为以下场景设计动效：页面切换、弹层出现/消失、列表新增/删除、加载状态、状态切换（开关/折叠）。装饰性动效应谨慎使用。'),
        ('如何测试动效性能？', '使用 Chrome DevTools Performance 面板录制动效帧，确保 60fps 不掉帧。使用 prefers-reduced-motion 测试关闭动效后的体验。'),
    ],
    'checks': [
        '动效时长在规范范围内',
...
NEW_CONTENT['ui']['ui/layout/'] = {
    'title': '布局系统规范',
    'overview': '统一的布局系统确保页面结构的一致性和可预测性。本规范定义了栅格系统、间距体系、布局模式，确保所有页面遵循统一的视觉节奏。',
    'principles': ['8px 基准：所有间距使用 8px 的倍数（8/16/24/32/48/64）', '栅格驱动：使用 12 列栅格系统组织页面布局', '层级明确：页面结构层次不超过 3 级（page → section → card）'],
    'rules': [
    ...
    'faqs': [
        ('什么时候用 Flex 什么时候用 Grid？', '一维布局（导航、按钮组、标签栏）用 Flexbox。二维布局（卡片网格、仪表盘、表格）用 CSS Grid。'),
...
NEW_CONTENT['ui']['ui/design-system/'] = {
    'title': '设计系统管理',
    'overview': '设计系统是产品团队的公共资产，需要版本化管理和持续维护。本规范定义了设计系统的版本策略、发布流程、文档标准和跨团队协作机制。',
    'principles': ['版本化管理：设计系统使用语义化版本号，变更留痕', '文档驱动：每个组件/模式必须有完整的设计和开发文档', '持续治理：定期审视设计系统中的组件使用率和废弃项'],
    'rules': [
    ...
    'faqs': [
        ('设计系统和组件库的区别？', '设计系统 = 组件库 + 设计令牌 + 设计模式 + 使用指南 + 工具链。组件库只是设计系统的一部分。'),
...
NEW_CONTENT['ui']['ui/accessibility/'] = {
    'title': '无障碍扩展实践',
    'overview': '在基础无障碍规范之上的高阶实践指南。涵盖复杂组件（数据表格、拖拽、树控件）、多媒体内容和单页应用路由管理等场景的无障碍实现方案。',
    'principles': ['超越合规：不仅满足 WCAG AA，追求 AAA 级体验', '原生优先：优先使用原生 HTML 语义元素，避免自定义组件', '渐进增强：基础功能不依赖 JavaScript，交互增强逐步添加'],
    'rules': [
    ...
    'faqs': [
        ('已有组件如何做无障碍改造？', '优先级：用户高频组件 > 表单组件 > 导航组件 > 数据展示组件。逐步修复，每次迭代 Fix 3-5 个组件。'),
        ('单页应用的路由管理怎么通知屏幕阅读器？', '使用 aria-live region 播报页面标题，同时用 JS 将焦点移到 main 区域开始处。使用 router 的 after hooks 实现。'),
    ],
    'checks': [
        '复杂组件 ARIA 属性完整',
        '数据表格行/列标题关联正确',
...
# ===== BUSINESS DOMAIN =====
NEW_CONTENT['business'] = {}
NEW_CONTENT['business']['business/doc-2/'] = {
    'title': '产品管理规范',
    'overview': '产品管理是连接业务目标与工程实现的核心环节。本规范定义了产品需求的生命周期管理流程，包括需求收集、评估、排期、跟踪和复盘，确保团队高效交付有价值的产品功能。',
    'principles': ['结果导向：每个需求必须有可量化的业务目标和成功指标', '持续交付：需求拆分为可独立交付的小单元，缩短发布周期', '数据驱动：产品决策基于用户数据和实验验证，而非直觉', '闭环反馈：上线后必须跟踪效果并迭代优化'],
    'rules': [
    ...
    'faqs': [
        ('如何确定需求的优先级？', '使用 RICE 框架（Reach × Impact × Confidence × Effort）评分排序。P0 为生死攸关需求，P1 为核心体验需求，P2 为体验优化需求，P3 为低优先级需求。'),
        ('需求文档应该多详细？', '核心需求提供完整 PRD（包含交互原型、业务规则、埋点方案）。小型需求使用精简模板，包含用户故事 + 验收标准即可。'),
        ('需求频繁变更是正常现象吗？', '变更是常态但不是随意变更的借口。采用迭代内冻结机制：迭代启动后不新增需求，变更放入 Backlog 等待下个迭代评估。'),
    ],
    'checks': [
        '需求有明确的业务目标和度量指标',
        '需求文档包含用户故事和验收标准',
...
NEW_CONTENT['business']['business/doc-3/'] = {
    'title': '业务战略规划',
    'overview': '业务战略规划定义产品的中长期发展方向和竞争策略。本规范涵盖市场分析、竞争定位、增长策略和战略执行框架，帮助团队做出正确的方向性决策。',
    'principles': ['方向聚焦：一次只做一件事，将资源集中在核心突破点上', '数据验证：所有战略假设必须通过小规模实验验证后再大规模投入', '敏捷调整：定期复盘战略进展，根据市场反馈及时调整方向', '上下对齐：战略目标从高层到底层逐层分解，确保全员方向一致'],
    'rules': [
    ...
    'faqs': [
        ('OKR 和 KPI 的区别？', 'OKR 是目标管理框架（Objectives and Key Results），用于设定方向性目标。KPI 是关键绩效指标，用于度量业务健康度。OKR 回答「我们要去哪里」，KPI 回答「我们现在在哪」。'),
        ('新兴市场如何做战略规划？', '采用「探索 → 验证 → 扩张」三阶段模型。探索期注重用户调研和问题验证，验证期注重 MVP 和产品市场匹配，扩张期注重规模化增长。'),
        ('竞品分析多久做一次？', '核心竞品持续跟踪（每周简报），重要竞品每季度深度分析。跟踪维度：产品功能、定价策略、市场份额、用户评价、融资动态。'),
    ],
    'checks': [
        '季度战略回顾完成',
        'OKR 已分解到团队/个人',
...
NEW_CONTENT['business']['business/doc-4/'] = {
    'title': '需求分析方法论',
    'overview': '需求分析是将模糊的业务诉求转化为清晰的系统需求的关键过程。本规范定义了需求获取、分析、建模和规格化的标准流程和工具方法，确保需求完整、一致、可测试。',
    'principles': ['用户为中心：从用户真实场景出发，而非从功能列表出发', '根本原因：多问为什么（5 Whys），找到需求的真实动机', '可验证：每个需求必须有明确的验收条件，确保可测试', '渐进细化：从粗到细逐层细化，不追求一次完美'],
    'rules': [
    ...
    'faqs': [
        ('用户故事和需求文档的关系？', '用户故事是需求的占位符，驱动对话。复杂的业务逻辑需要独立的 PRD 文档进行详细描述。简单需求可以用户故事 + 验收条件直接进入开发。'),
        ('需求分析中最常见的问题？', '① 假设用户和实际用户不一致（没有做用户调研）② 把解决方案当需求（用户说「我要一个导出按钮」其实需要的是「我要把数据导入 Excel」）③ 忽略非功能需求（只关注功能不关心性能）。'),
    ],
    'checks': [
        '需求有明确的业务价值',
...
NEW_CONTENT['business']['business/domain-events/'] = {
    'title': '市场研究方法',
    'overview': '市场研究是了解用户需求、竞争环境和市场趋势的系统性方法。本规范定义了定量研究、定性研究、竞争分析和行业研究的标准流程和工具选择指南。',
    'principles': ['方法匹配：研究方法的选择取决于决策类型和不确定性程度', '样本有效：确保研究样本能够代表目标用户群体', '避免偏差：研究设计和执行过程中识别并最小化各类偏差', '可操作：研究结论必须转化为可执行的产品或业务决策'],
    'rules': [
    ...
    'faqs': [
        ('定性研究和定量研究怎么选？', '回答「为什么」用定性研究（用户访谈、可用性测试），回答「有多少」用定量研究（问卷调查、A/B 测试、数据分析）。通常先用定性探索，再用定量验证。'),
        ('如何招募研究参与者？', '从真实用户池中招募，避免使用同事或朋友。提供合理的激励（约 100-200 元/小时）。每个研究阶段招募 8-12 人进行访谈，300+ 人进行问卷。'),
    ],
    'checks': [
        '研究目的和方法明确',
        '样本量达到最低要求',
...
NEW_CONTENT['business']['business/state-machine/'] = {
    'title': 'KPI 指标体系',
    'overview': 'KPI 指标体系是度量业务健康度和团队绩效的量化工具。本规范定义了指标的分类、选择标准、数据采集和可视化规范，确保团队聚焦在真正重要的度量上。',
    'principles': ['可度量：每个指标必须有明确的计算公式和数据来源', '可行动：指标变化应能指导具体的改进行动', '平衡全面：覆盖业务各维度，避免单一指标驱动', '分级管理：北极星指标 → 一级指标 → 二级指标三级联动'],
    'rules': [
    ...
    'faqs': [
        ('如何选择北极星指标？', '北极星指标应反映产品为用户创造的核心价值。好指标特征：① 用户价值相关 ② 可度量 ③ 可影响 ④ 长期导向。例如 Spotify 的「付费用户月活跃天数」、Airbnb 的「预订间夜数」。'),
        ('指标太多怎么办？', '采用「三个层级」：高层关注北极星指标和一级指标（月度回顾），中层关注二级指标（周度回顾），执行层关注过程指标（每日/实时）。'),
    ],
    'checks': [
        '北极星指标已定义',
        '一级指标 ≤ 5 个',
...
NEW_CONTENT['business']['business/reporting/'] = {
    'title': '用户增长策略',
    'overview': '用户增长是通过系统化的实验和方法推动用户获取、激活、留存和推荐的过程。本规范定义了增长实验框架、渠道评估标准和增长模型选择指南。',
    'principles': ['实验驱动：每个增长假设必须通过 A/B 实验验证', '漏斗思维：分析每个环节的转化率，找到最大改善空间', '激活优先：先让新用户体验到核心价值（Aha Moment），再谈留存', '可持续增长：关注有机增长（口碑/推荐）而非付费获取'],
    'rules': [
    ...
    'faqs': [
        ('增长黑客和传统营销的区别？', '增长黑客更注重产品驱动的增长（Product-Led Growth），通过优化产品体验本身来驱动获客和留存，而非依靠外部营销投放。核心方法论是构建增长飞轮。'),
        ('LTV 和 CAC 的合理比例？', 'SaaS 行业最佳实践：LTV:CAC ≥ 3:1。小于 3:1 说明获客成本过高，大于 7:1 说明可能在增长上投入不足。CAC 回收期应小于 12 个月。'),
    ],
    'checks': [
        '增长假设文档化',
        'A/B 实验达到统计显著',
...
NEW_CONTENT['business']['business/audit/'] = {
    'title': '数据驱动决策',
    'overview': '数据驱动决策是通过系统化的数据采集、分析和实验来指导产品和业务决策的方法论。本规范定义了数据文化建设的标准流程、分析框架和决策质量要求。',
    'principles': ['数据为王：重大决策必须有数据支持，不能仅凭直觉', '相关性 ≠ 因果性：谨慎区分相关关系和因果关系', '透明可复现：分析过程和结论必须可复现、可审查', '行动导向：分析的最终目的是指导行动，而非展示数据'],
    'rules': [
    ...
    'faqs': [
        ('没有足够数据的时候怎么做决策？', '采用「高质量决策框架」：① 明确决策标准和权重 ② 收集可获得的定性数据 ③ 识别关键假设 ④ 小步验证关键假设。没有完美数据不等于不能做决策。'),
        ('怎么建立数据驱动的团队文化？', '三步走：① 提供自助 BI 工具让每个人都能查数据 ② 建立数据字典和指标口径文档 ③ 在决策流程中强制加入数据环节（每个需求必须有数据支撑）。'),
    ],
    'checks': [
        '决策有数据支持',
        '实验/评估方案已设计',
...
NEW_CONTENT['business']['business/retry/'] = {
    'title': '敏捷开发流程',
    'overview': '敏捷开发是一种迭代增量的软件开发方法，强调快速交付、持续反馈和团队自组织。本规范定义了 Sprint 周期、站会、评审、回顾等 Scrum 实践的标准和最佳做法。',
    'principles': ['迭代交付：每 1-2 周交付一个可用的产品增量', '持续反馈：每个迭代结束后进行评审和回顾，快速调整', '团队自组织：团队自主认领任务和决定实现方式', '技术卓越：持续关注代码质量和工程实践，不欠技术债'],
    'rules': [
        ('AP-001', 'Sprint 周期固定为 1 或 2 周，不可随意延长或缩短', 'P0', '是'),
        ('AP-002', '每日站会不超过 15 分钟，只说进度、计划和阻碍', 'P0', '是'),
        ('AP-003', 'Sprint Planning 必须定义 Sprint Goal 和承诺的 Story Points', 'P0', '是'),
    ...
    'faqs': [
        ('Scrum 和 Kanban 怎么选？', 'Scrum 适合需求相对稳定的产品开发（固定迭代周期、团队角色明确）。Kanban 适合需求变化频繁的运维和支持类工作（不限周期的持续交付）。团队成熟后可采用 Scrumban 混合模式。'),
        ('Velocity 下降怎么办？', '首先分析原因（技术债务、需求复杂度增加、团队变更等），而非强制要求提升 Velocity。健康的团队应关注价值交付而非速度数字。'),
    ],
    'checks': [
        'Sprint 周期固定',
        '每日站会按时进行',
...
NEW_CONTENT['business']['business/rule-engine/'] = {
    'title': '项目管理规范',
    'overview': '项目管理是确保项目在时间、预算和质量约束下成功交付的系统方法。本规范涵盖了项目启动、计划、执行、监控和收尾的全流程管理标准和里程碑跟踪方法。',
    'principles': ['目标明确：项目启动前必须有明确的范围、目标和成功标准', '计划先行：有效的计划胜过临时的应对', '风险前置：提前识别和应对风险，而非问题发生后再补救', '透明沟通：项目状态对所有人可见，问题和变更及时通报'],
    'rules': [
    ...
    'faqs': [
        ('如何应对项目延期？', '① 识别关键路径上哪些任务在延迟 ② 评估是否可以并行执行、增加资源或缩减范围 ③ 与管理层沟通方案和影响 ④ 更新项目计划。核心原则：不要通过降低质量来追赶进度。'),
        ('远程团队的项目管理有什么不同？', '加强异步沟通（文档化所有决策）、使用项目管理工具（Jira/Asana/Notion）透明跟踪、定期 1:1 沟通、建立虚拟饮水机时间。'),
    ],
    'checks': [
        '项目章程已完成',
        '里程碑计划已制定',
...
NEW_CONTENT['business']['business/risk-management/'] = {
    'title': '风险管理规范',
    'overview': '风险管理是识别、评估和应对项目或业务中潜在不确定性的系统过程。本规范定义了风险分类、评估矩阵、应对策略和监控机制，帮助团队主动管理而非被动应对。',
    'principles': ['前置预防：防患于未然，风险识别在项目启动阶段就开始', '量化评估：使用概率 × 影响矩阵对风险进行量化排序', '主动应对：每个高风险必须有明确的应对策略和负责人', '持续监控：风险是动态的，需要定期复审和更新'],
    'rules': [
        ('RSK-001', '所有项目必须建立风险登记册（Risk Register）', 'P0', '是'),
        ('RSK-002', '风险按照概率×影响矩阵评级：高（H）、中（M）、低（L）', 'P0', '是'),
        ('RSK-003', '高风险必须有应对计划（规避/转移/减轻/接受之一）', 'P0', '是'),
        ('RSK-004', '风险登记册每两周审查更新一次', 'P0', '是'),
        ('RSK-005', '风险缓解措施需设置检查点和触发条件', 'P1', '是'),
        ('RSK-006', '已关闭的风险需要记录实际发生情况和经验教训', 'P1', '推荐'),
    ],
    'faqs': [
        ('如何判断风险概率和影响？', '通过历史数据、专家评估和同类项目参考。概率分五级：几乎不可能(<10%)、不太可能(10-30%)、可能(30-50%)、很可能(50-80%)、几乎确定(>80%)。影响分五级：轻微、较小、中等、重大、灾难。'),
        ('风险发生后的应急流程？', '① 立即通知风险管理负责人 ② 执行已制定的应对计划 ③ 评估实际影响并记录 ④ 更新风险登记册 ⑤ 召开复盘会议。所有步骤需在 24h 内完成。'),
    ],
    'checks': [
        '风险登记册已建立',
        '风险按概率×影响评估',
    ...
    'prompt_sp': '你是一个风险管理专家，精通项目风险和业务风险评估与控制。',
    'prompt_up': '请为以下场景进行风险评估和管理方案设计：\n项目/业务描述：{project_desc}\n已知风险：{known_risks}\n风险承受度：{risk_appetite}',
    'example_text': '```markdown\n# 风险登记册：ERP 系统升级\n\n## 风险矩阵\n| 风险 | 概率 | 影响 | 等级 | 应对策略 | 负责人 |\n|------|------|------|------|----------|--------|\n| 数据迁移丢失 | 30% | 严重 | H | 减轻：全量备份+校验 | 张三 |\n| 系统停机超时 | 20% | 严重 | H | 减轻：灰度切换+回滚方案 | 李四 |\n| 用户培训不足 | 60% | 中等 | M | 减轻：分批培训+操作手册 | 王五 |\n| 第三方API变更 | 40% | 较小 | M | 转移：合同中锁定版本 | 赵六 |\n| 硬件采购延迟 | 15% | 轻微 | L | 接受：提前采购 | 钱七 |\n\n## 高风险应对计划\n\n### 风险 A: 数据迁移丢失\n- 应对类型: 减轻\n- 应对措施:\n  1. 迁移前全量备份\n  2. 分批迁移（每批 10%）\n  3. 每批迁移后自动校验数据一致性\n  4. 校验失败自动回滚该批次\n- 触发条件: 校验发现数据不一致\n- 负责人: 张三\n- 检查点: 每批次迁移完成\n\n### 风险 B: 系统停机超时\n- 应对类型: 减轻\n- 应对措施:\n  1. 灰度切换（10% → 30% → 100%）\n  2. 每个阶段保留 30min 观察期\n  3. 准备完整的回滚剧本\n- 触发条件: 停机超过计划时间 50%\n- 负责人: 李四\n```',
    'example_ext': 'md',
}

# ===== DATABASE DOMAIN =====
NEW_CONTENT['database'] = {}
NEW_CONTENT['database']['database/relational-design/'] = {
    'title': '关系型数据库设计规范',
    'overview': '关系型数据库设计是系统架构的基石，直接影响数据一致性、查询性能和业务扩展能力。本规范涵盖表结构设计、索引策略、ER 建模和命名规则。',
    'principles': ['范式适可：三范式为基础，根据查询性能需求合理反范式化', '主键设计：使用自增 ID 或 UUID 作为主键，业务字段不做主键', '索引有度：索引不是越多越好，权衡查询加速和写入开销', '命名清晰：表和字段命名自解释，避免缩写歧义'],
    'rules': [
    ...
    'faqs': [
        ('自增 ID 还是 UUID？', '自增 ID：性能好、索引小、适合内部系统。UUID：适合分布式系统、避免暴露业务量级。折衷方案：使用雪花算法（Snowflake ID）生成分布式唯一 ID。'),
        ('反范式化什么时候用？', '当查询性能成为瓶颈，且数据一致性可以通过应用层保证时。典型场景：冗余展示字段（如订单表中的用户名称）、预计算聚合（如文章表中的评论数）。'),
        ('删除数据是物理删除还是逻辑删除？', '推荐逻辑删除（is_deleted 字段），保留数据可追溯。对于需要物理删除的场景（如 GDPR 合规），使用定时清理任务批量处理。'),
    ],
    'checks': [
        '每张表有主键',
...
NEW_CONTENT['database']['database/sql/'] = {
    'title': 'SQL 优化指南',
    'overview': 'SQL 优化是提升数据库查询性能的核心手段。本规范涵盖了执行计划分析、索引优化、查询重写和慢查询治理的最佳实践。',
    'principles': ['索引先行：90% 的查询性能问题可以通过合适的索引解决', '数据少取：只返回需要的列和行，避免 SELECT *', '避免全表扫描：大表查询必须走索引', '持续治理：慢查询日志必须持续监控和优化'],
    'rules': [
    ...
    'faqs': [
        ('如何分析 SQL 性能？', '使用 EXPLAIN ANALYZE（MySQL 8.0.18+ / PostgreSQL）查看实际执行计划和耗时。关注 type 字段（ALL=全表扫描需优化）、rows（扫描行数）和 Extra（Using filesort/Using temporary 需优化）。'),
        ('索引加了查询还是慢？', '可能原因：① 索引选择性差（重复值太多）② WHERE 条件用了 OR 导致索引失效 ③ 查询返回行数过多 ④ 索引碎片严重（需重建）。'),
        ('LIKE 模糊查询能用索引吗？', '前缀匹配（LIKE \'abc%\'）可以用索引。后缀匹配（LIKE \'%abc\'）和包含匹配（LIKE \'%abc%\'）不能用 B+Tree 索引。后缀匹配需求使用倒排索引（全文索引或 Elasticsearch）。'),
    ],
    'checks': [
        'SELECT 语句无星号查询',
        'WHERE 条件索引列无函数包裹',
...
NEW_CONTENT['database']['database/sharding/'] = {
    'title': 'NoSQL 数据库设计',
    'overview': 'NoSQL 数据库在特定场景下提供比关系型数据库更好的扩展性和性能。本规范涵盖了 MongoDB、Redis、Cassandra 的适用场景、数据模型设计和最佳实践。',
    'principles': ['场景匹配：根据数据模型和访问模式选择最合适的 NoSQL 类型', '文档内嵌：MongoDB 优先内嵌而非引用，减少跨文档查询', '反范式常态：NoSQL 设计中数据冗余是可接受的', 'TTL 策略：缓存和临时数据必须设置过期时间'],
    'rules': [
    ...
    'faqs': [
        ('什么时候用 MongoDB 什么时候用 MySQL？', 'MongoDB 适合：文档数据、灵活 Schema、快速迭代。MySQL 适合：强一致性需求、复杂 JOIN 和事务、结构化数据。一个好的架构经常同时使用两者（Polyglot Persistence）。'),
        ('Redis 缓存什么数据？', '热点数据（频繁访问但不常变更）、Session 数据、计数器（点赞/阅读量）、分布式锁、限流器。不适合：全量数据、需要持久化保证的数据、大 Key（>10MB）。'),
    ],
    'checks': [
        'NoSQL 选型有合理依据',
        'MongoDB 文档嵌套 ≤ 3 层',
...
NEW_CONTENT['database']['database/doc-3/'] = {
    'title': '数据建模方法',
    'overview': '数据建模是将现实世界的业务需求抽象为数据结构和关系的过程。本规范涵盖了概念模型、逻辑模型和物理模型的分层设计方法，确保数据模型与业务语义一致。',
    'principles': ['业务驱动：数据模型反映业务概念和规则，而非数据库技术限制', '分层设计：概念 → 逻辑 → 物理 逐层细化和转换', '命名统一：统一数据字典，消除同名异义和异名同义', '可追溯：每个数据元素都可追溯到业务需求来源'],
    'rules': [
    ...
    'faqs': [
        ('概念模型和逻辑模型的区别？', '概念模型：用业务语言描述实体和关系，不含技术细节（如 ER 图中的业务实体）。逻辑模型：添加属性、主键、外键、数据类型等细节，与具体数据库无关。物理模型：针对特定数据库的优化（索引、分区、存储引擎）。'),
        ('数据字典应该包含什么？', '字段名、数据类型、长度、是否必填、默认值、取值范围、业务含义、示例值、来源系统。统一的数据字典是数据治理的基础。'),
    ],
    'checks': [
        '数据字典文档已建立',
        '核心实体有 ER 图',
...
NEW_CONTENT['database']['database/doc-2/'] = {
    'title': '数据库迁移策略',
    'overview': '数据库迁移是软件系统演进中不可避免的操作，包括 Schema 变更、数据迁移和版本升级。本规范定义了迁移流程、回滚策略和零停机迁移方案。',
    'principles': ['先读后写：先确保新数据可读，再切换到新写入路径', '可回滚：每次迁移都必须有经过验证的回滚方案', '小步快跑：每次迁移变更小、风险低、易于隔离和排查', '数据校验：迁移前后必须对数据一致性进行全面校验'],
    'rules': [
    ...
    'faqs': [
        ('零停机迁移怎么做？', '采用「并行双写 → 数据同步 → 逐步切流」策略：① 新旧库同时写入 ② 后台同步历史数据并校验 ③ 读流量逐步切到新库 ④ 观察无问题后下线旧库。整个过程 1-2 周。'),
        ('迁移失败怎么办？', '立即触发回滚流程：① 停止迁移脚本 ② 执行回滚脚本 ③ 校验数据一致性 ④ 业务降级（如切换到只读模式） ⑤ 排查原因后重新计划。'),
        ('大表加索引会锁表吗？', 'MySQL 5.6+ 支持 ONLINE DDL，但大表加索引仍有性能影响。推荐使用 pt-osc（Percona Toolkit Online Schema Change）在从库或低峰期执行。'),
    ],
    'checks': [
        '迁移脚本版本化',
        '回滚脚本已准备',
...
NEW_CONTENT['database']['database/backup/'] = {
    'title': '备份与恢复策略',
    'overview': '数据备份是防止数据丢失的最后一道防线。本规范定义了备份策略的制定标准、备份类型的选择、恢复演练流程和 RPO/RTO 的设定方法。',
    'principles': ['3-2-1 原则：至少 3 份备份，2 种不同介质，1 份异地存储', '定期验证：备份不等于安全，定期做恢复测试', '自动化：备份和验证流程全自动化，减少人为失误', '分级保护：根据数据重要性级别制定差异化备份策略'],
    'rules': [
    ...
    'faqs': [
        ('RPO 和 RTO 怎么定？', 'RPO（恢复点目标）决定可容忍的数据丢失量，如 RPO=1h 意味着最多丢失 1 小时数据。RTO（恢复时间目标）决定可容忍的停机时间。核心业务建议 RPO ≤ 15min，RTO ≤ 30min。'),
        ('全量备份和增量备份的策略？', '推荐「全量+增量」组合：每周日全量备份，周一至周六增量备份。恢复时先恢复最近的全量备份，再依次应用增量备份。'),
...
NEW_CONTENT['database']['database/doc-5/'] = {
    'title': '数据仓库设计规范',
    'overview': '数据仓库是企业数据分析和 BI 报告的基础设施。本规范涵盖了数据分层架构（ODS → DWD → DWS → ADS）、维度建模方法（星型/雪花型）和 ETL/ELT 的设计标准。',
    'principles': ['分层清晰：ODS-DWD-DWS-ADS 四层职责分明，数据逐层聚合', '一致性维度：核心维度（时间、用户、产品）在全局保持一致', '缓慢变化维：根据业务需求选择 SCD Type 1/2/3', '数据质量：每层的数据质量检查是进入下一层的前提'],
    'rules': [
    ...
    'faqs': [
        ('星型模型和雪花模型怎么选？', '星型模型：维度冗余（宽表），查询性能好，适合 BI 工具。雪花模型：维度规范化，存储少，适合 OLAP 场景。推荐优先使用星型模型，需要节省存储时才用雪花模型。'),
        ('数据湖和数据仓库的区别？', '数据湖存储原始格式数据（结构化+半结构化+非结构化），Schema-on-Read。数据仓库存储处理后的结构化数据，Schema-on-Write。现代架构中常同时使用两者（Lake House）。'),
    ],
    'checks': [
        '数据分层架构明确',
...
# ===== API DOMAIN =====
NEW_CONTENT['api'] = {}
NEW_CONTENT['api']['api/restful-api/'] = {
    'title': 'RESTful API 设计规范',
    'overview': 'RESTful API 是微服务架构中最常见的服务间通信方式。本规范定义了 URI 设计、HTTP 方法使用、状态码选择、请求/响应格式和错误处理的标准。',
    'principles': ['资源导向：API 围绕资源而非动作设计', '自描述：每个请求和响应包含足够的上下文信息', '一致性：所有 API 遵循统一的命名、格式和行为约定', '版本兼容：向后兼容性优先，破坏性变更必须发新版本'],
    'rules': [
    ...
    'faqs': [
        ('PUT 和 PATCH 的区别？', 'PUT 是幂等的全量替换，PATCH 是非幂等的部分更新。例如更新用户信息：PUT 需要传全部字段（未传的字段重置为默认值），PATCH 只需传需要修改的字段。'),
        ('RESTful API 怎么处理批量操作？', '使用自定义端点：POST /api/batch/users（批量创建），或者使用查询参数：DELETE /api/users?ids=1,2,3（批量删除）。不建议在标准资源端点上做批量操作。'),
        ('API 响应中应该包含多少数据？', '默认响应包含核心字段（id、name、status 等）。如果客户端需要完整数据，使用查询参数 ?fields=all 或 ?include=detail,stats 按需加载。'),
    ],
    'checks': [
        'URI 使用名词复数',
        'HTTP 方法和状态码使用正确',
...
NEW_CONTENT['api']['api/openapi/'] = {
    'title': 'GraphQL API 设计规范',
    'overview': 'GraphQL 提供比 REST 更灵活的数据查询能力，客户端可以精确指定需要的数据字段。本规范定义了 Schema 设计、Query/Mutation 拆分、N+1 优化和权限控制的标准。',
    'principles': ['Schema 驱动：API 行为由 Schema 定义，前后端基于 Schema 协作', '按需查询：客户端精确指定所需字段，避免数据过载', '批量化：使用 DataLoader 解决 N+1 查询问题', '安全第一：查询深度和复杂度限制是必需的'],
    'rules': [
    ...
    'faqs': [
        ('GraphQL 和 REST 怎么选？', 'GraphQL 适合：数据聚合层（BFF）、前端驱动的快速迭代、多客户端差异需求。REST 适合：简单 CRUD、缓存友好、工具链成熟、第三方 API。实践中常混合使用两种方式。'),
        ('GraphQL 的性能问题怎么处理？', '① DataLoader 解决 N+1 ② Persisted Queries 减少传输 ③ @defer/@stream 指令优化大数据加载 ④ CDN 缓存查询结果 ⑤ 使用 Apollo Studio 监控慢查询。'),
    ],
    'checks': [
        '查询深度限制已配置',
        'Mutation 幂等性实现',
...
NEW_CONTENT['api']['api/api/'] = {
    'title': 'API 安全规范',
    'overview': 'API 安全是系统安全的第一道防线。本规范涵盖了认证鉴权、输入校验、速率限制、数据保护和审计日志等安全控制措施。',
    'principles': ['纵深防御：多层安全控制，不依赖单一防护手段', '最小权限：每个 API 只暴露必要的数据和操作', '默认拒绝：未明确授权的请求一律拒绝', '安全 First：安全性在 API 设计阶段就要考虑，而非上线前补充'],
    'rules': [
    ...
    'faqs': [
        ('JWT Token 应该存在哪里？', 'Web 端使用 httpOnly Cookie（防 XSS 窃取），移动端使用安全存储（iOS Keychain / Android EncryptedSharedPreferences）。不要使用 localStorage（易受 XSS 攻击）。'),
        ('如何防止 API 被恶意调用？', '① IP 级别和用户级别的双维度限流 ② 查接口频次异常告警 ③ CAPTCHA 验证 ④ API 签名校验（如 HMAC）⑤ 敏感操作需二次确认。'),
        ('CORS 配置有什么注意的？', '不要使用 Access-Control-Allow-Origin: *。明确指定允许的域名来源。不要在响应中暴露认证相关的 Header。不要在 OPTIONS 预检请求中泄露敏感信息。'),
    ],
    'checks': [
        'HTTPS 已强制开启',
        'OAuth 2.0 / JWT 认证已实现',
...
NEW_CONTENT['api']['api/doc-3/'] = {
    'title': 'API 版本管理规范',
    'overview': 'API 版本管理确保在不破坏现有客户端的情况下持续演进 API。本规范定义了版本策略、兼容性保障、废弃流程和版本生命周期管理标准。',
    'principles': ['兼容优先：优先考虑向后兼容，版本升级不是破坏的借口', '渐进废弃：给客户端充分的迁移时间，至少 6 个月过渡期', '可发现：客户端能方便地发现 API 版本信息和变更日志', '标准化：版本策略在所有微服务中统一执行'],
    'rules': [
    ...
    'faqs': [
        ('哪些属于破坏性变更？', '① 删除/重命名字段 ② 修改字段类型 ③ 修改端点 URL ④ 修改认证方式 ⑤ 修改错误响应结构 ⑥ 修改请求参数语义。新增字段、新增端点属于非破坏性变更。'),
        ('如何处理多个 API 版本共存？', '所有版本部署在同一服务上（代码分支或路由分发），共享数据层。维护 2-3 个活跃版本（当前版本 + 1-2 个旧版本），超过 3 个版本说明废弃流程有问题。'),
    ],
    'checks': [
        'API 版本号在 URL 路径中',
        '废弃通知已提前发出',
...
NEW_CONTENT['api']['api/doc-4/'] = {
    'title': 'API 文档规范',
    'overview': '良好的 API 文档是开发者体验的核心组成部分。本规范定义了 API 文档的内容标准、格式要求和发布流程，确保文档完整、准确、易于使用。',
    'principles': ['文档即代码：API 文档与代码一同维护、一同评审、一同发布', '入门友好：新开发者应在 5 分钟内完成第一个 API 调用', '示例驱动：每个 API 必须有请求/响应示例，覆盖常见场景', '持续更新：API 变更必须同步更新文档，文档评审是 CI 流程的一环'],
    'rules': [
    ...
    'faqs': [
        ('API 文档工具选什么？', 'Swagger/OpenAPI 是事实标准。前端推荐：Swagger UI（在线文档）、Redoc（静态文档）。基于代码生成：FastAPI（自动生成）、SpringDoc（Java）、drf-yasg（Django）。'),
        ('文档应该多详细？', '核心：端点用途、请求参数（类型/必填/默认）、响应结构（HTTP 状态码对应）、错误码。进阶：速率限制说明、版本历史、迁移指南、常见问题。'),
    ],
    'checks': [
        'OpenAPI 3.0+ 文档已生成',
        '每个端点有完整描述',
...
# ===== TEST DOMAIN =====
NEW_CONTENT['test'] = {}
NEW_CONTENT['test']['test/doc-2/'] = {
    'title': '单元测试规范',
    'overview': '单元测试是保证代码质量和防止回归的第一道防线。本规范定义了单元测试的编写标准、覆盖率要求、Mock 策略和命名规范，确保测试的可维护性和有效性。',
    'principles': ['行为验证：测试外部行为而非内部实现，避免脆弱的测试', '单一断言：每个测试方法聚焦验证一个行为', 'FIRST 原则：Fast（快速）、Isolated（隔离）、Repeatable（可重复）、Self-validating（自验证）、Timely（及时）'],
    'rules': [
    ...
    'faqs': [
        ('覆盖率 100% 有必要吗？', '没有必要且有害。100% 覆盖率通常会导致测试关注实现细节而非行为。核心业务逻辑 90%+，工具类/非关键模块 60-80% 是合理的。'),
        ('如何测试异步代码？', '使用 async/await 测试异步方法。使用超时断言防止死锁。使用 TestScheduler 控制时间相关的异步操作。'),
        ('Mock 太多的代码设计有什么问题？', '需要大量 Mock 往往意味着代码耦合度高、职责不单一。这是重构信号：考虑拆分类、使用依赖注入、减少静态方法调用。'),
    ],
    'checks': [
        '测试与代码一起提交',
        '核心逻辑覆盖率 ≥ 90%',
...
NEW_CONTENT['test']['test/doc-3/'] = {
    'title': '集成测试规范',
    'overview': '集成测试验证多个组件或系统之间的交互是否正确。本规范涵盖了数据库集成测试、外部服务 Mock/Stub、测试容器使用和测试环境管理。',
    'principles': ['真实优先：对存储层（数据库/缓存）使用真实实例而非 Mock', '独立环境：每次测试运行使用独立的数据库/容器', '最小化外部依赖：对外部 API 使用 WireMock 或契约测试', '数据隔离：测试之间不共享测试数据，每个测试自包含'],
    'rules': [
    ...
    'faqs': [
        ('集成测试应该覆盖哪些场景？', '重点覆盖：① 数据持久化和检索 ② 消息队列收发 ③ 外部 API 调用链路 ④ 事务回滚和一致性 ⑤ 异常处理路径。不覆盖：复杂业务逻辑（单元测试覆盖）、UI 交互（E2E 覆盖）。'),
        ('测试数据库用 H2 还是真实数据库？', '推荐使用真实数据库（PostgreSQL/MySQL via Testcontainers）。H2 兼容性不完全，可能导致测试通过但生产环境出错。'),
    ],
    'checks': [
...
NEW_CONTENT['test']['test/e-e/'] = {
    'title': '端到端测试规范',
    'overview': 'E2E 测试从用户视角验证完整的业务流程，确保系统各模块协同工作。本规范定义了测试场景选择、自动化框架选择和 CI 集成标准。',
    'principles': ['用户视角：以用户操作路径设计测试用例，不关注技术实现', '核心优先：只覆盖核心业务路径，不追求全量覆盖', '稳定可靠：使用 Data Test ID 而非 CSS/XPath 选择器', '分层运行：关键路径运行在 CI 中，全量回归定期运行'],
    'rules': [
    ...
    'faqs': [
        ('E2E 测试失败率很高怎么办？', '首先区分「被测系统 Bug」和「测试 flakiness」。flakiness 通常由定时问题、异步等待不足、测试数据冲突导致。使用 Retry 机制（重试 2-3 次）和重试间隔解决。'),
        ('Playwright vs Cypress vs Selenium？', 'Playwright（推荐）：跨浏览器、速度快、API 现代化、支持移动端。Cypress：调试体验好、社区大、但只支持 Chrome。Selenium：老牌工具、生态丰富、但速度慢。'),
    ],
    'checks': [
        '核心业务路径有 E2E 覆盖',
        '使用 data-testid 定位元素',
...
NEW_CONTENT['test']['test/doc-5/'] = {
    'title': '性能测试规范',
    'overview': '性能测试确保系统在高负载下仍能稳定运行并满足响应时间要求。本规范涵盖了负载测试、压力测试、耐久测试和容量规划的标准方法和工具选择。',
    'principles': ['早期介入：性能测试从开发阶段就开始，而非上线前才做', '可量化：所有性能指标必须有明确的 SLO（服务等级目标）', '基准对比：每次性能测试都与历史基线对比，识别退化', '真实场景：测试负载模型基于生产环境流量模式'],
    'rules': [
    ...
    'faqs': [
        ('JMeter 还是 Locust 还是 k6？', 'k6（推荐）：Go 开发、配置简单、CI 集成好、支持 JavaScript 脚本。JMeter：老牌工具、功能全面、但资源占用大。Locust：Python 编写、分布式支持好。'),
...
NEW_CONTENT['test']['test/tdd/'] = {
    'title': '测试驱动开发方法论',
    'overview': '测试驱动开发（TDD）是编写生产代码前先写测试的开发方法。本规范定义了 TDD 的红-绿-重构循环、节奏控制和工具链配置。',
    'principles': ['测试先行：在写实现代码之前先写测试', '最简实现：只写让测试通过的最少代码，不提前设计', '持续重构：测试通过后立即重构，保持代码整洁', '小步前进：每次只做一点点改动，保持测试一直通过'],
    'rules': [
    ...
    'faqs': [
        ('TDD 真的能提高效率吗？', '初期因为写测试会慢 10-20%，但在项目中期和后期，由于回归 Bug 大幅减少和代码可维护性提升，整体效率显著提高。核心收益是减少调试时间。'),
        ('什么时候不适合 TDD？', '探索性原型、UI 设计阶段、一次性的脚本代码。但这些情况也可以用 TDD 的思想：「先把预期行为描述清楚再编码」。'),
        ('TDD 和前段开发怎么结合？', '组件使用 Storybook（Visual TDD），业务逻辑使用 Jest/Testing Library，E2E 使用 Playwright。测试可以替换「手动刷新浏览器验证」的流程。'),
    ],
    'checks': [
        '遵循红-绿-重构循环',
        '测试先于实现代码',
...
NEW_CONTENT['test']['test/doc-6/'] = {
    'title': '测试自动化规范',
    'overview': '测试自动化是将测试执行从手工转为自动化的系统过程。本规范定义了自动化测试的分层策略、用例选择标准、维护要求和 CI/CD 集成标准。',
    'principles': ['测试金字塔：单元测试占 70%、集成测试占 20%、E2E 测试占 10%', '稳定优先：自动化测试的稳定性比覆盖率更重要', '可维护：测试代码与生产代码一样需要评审和维护', '价值导向：自动化的价值 = (节省的时间 × 运行次数) - 维护成本'],
    'rules': [
    ...
    'faqs': [
        ('测试增量维护的工作量怎么估计？', '通常每 100 行生产代码需要 50-80 行测试代码。测试维护成本大约是生产代码的 1.5 倍。关键是不让测试变成负债——及时修复 flaky 测试、删除不再适用的测试。'),
        ('如何决定哪些测试需要自动化？', '三个条件：① 手动执行频率高（至少每周跑一次）② 执行结果可自动校验 ③ 自动化 ROI 为正（节省时间 > 维护成本）。不满足条件的场景保持手工测试。'),
    ],
    'checks': [
        'CI 中集成自动化测试',
        '测试分层比例合理',
...
# ===== DEPLOY DOMAIN =====
NEW_CONTENT['deploy'] = {}
NEW_CONTENT['deploy']['deploy/doc-2/'] = {
    'title': 'CI/CD 流水线规范',
    'overview': 'CI/CD 流水线是软件交付的核心基础设施，实现代码从提交到生产的自动化交付。本规范定义了流水线阶段、质量标准、部署策略和环境管理标准。',
    'principles': ['自动化一切：从代码检查到部署全流程自动化，减少人为操作', '质量门禁：每个阶段设置质量门禁，不合格的代码不允许进入下一阶段', '快速反馈：流水线应在 15 分钟内给出完整的质量反馈', '不可变部署：每次部署生成不可变版本，避免运行时修改'],
    'rules': [
    ...
    'faqs': [
        ('Jenkins vs GitHub Actions vs GitLab CI？', 'GitHub Actions（推荐）：与 GitHub 深度集成、Action 生态丰富、配置简单。GitLab CI：内置容器注册中心、Kubernetes 集成好。Jenkins：高度可定制、插件生态最大、但维护成本高。'),
        ('如何优化 CI 速度？', '① 构建缓存（依赖缓存、Docker 缓存层）② 并行执行（分段并行测试）③ 增量构建（只编译变更部分）④ 按需运行（前端变更时只跑前端测试）⑤ 使用高性能 Runner。'),
    ],
    'checks': [
        '每次提交触发 CI',
        'CI 完成时间在 10 分钟内',
    ...
    'example_text': "```yaml\n# .github/workflows/deploy.yml\nname: Build and Deploy\n\non:\n  push:\n    branches: [main]\n\nenv:\n  REGISTRY: ghcr.io\n  IMAGE_NAME: ${{ github.repository }}\n\njobs:\n  build-and-test:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n      - uses: actions/setup-node@v4\n        with:\n          node-version: '20'\n          cache: 'npm'\n      - run: npm ci\n      - run: npm run lint\n      - run: npm run type-check\n      - run: npm test -- --coverage\n      - run: npm run build\n\n  docker-build-and-push:\n    needs: build-and-test\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n      - run: |\n          echo \"${{ secrets.GITHUB_TOKEN }}\" | docker login ghcr.io -u ${{ github.actor }} --password-stdin\n          docker build -t $REGISTRY/$IMAGE_NAME:${{ github.sha }} .\n          docker tag $REGISTRY/$IMAGE_NAME:${{ github.sha }} $REGISTRY/$IMAGE_NAME:latest\n          docker push $REGISTRY/$IMAGE_NAME:${{ github.sha }}\n          docker push $REGISTRY/$IMAGE_NAME:latest\n\n  deploy-staging:\n    needs: docker-build-and-push\n    environment: staging\n    runs-on: ubuntu-latest\n    steps:\n      - run: |\n          curl -X POST ${{ secrets.DEPLOY_WEBHOOK }} \\\n            -H \"Content-Type: application/json\" \\\n            -d '{\"image\": \"${{ env.REGISTRY }}/${{ env.IMAGE_NAME }}:${{ github.sha }}\", \"env\": \"staging\"}'\n\n  deploy-production:\n    needs: deploy-staging\n    environment: production\n    runs-on: ubuntu-latest\n    steps:\n      - run: |\n          curl -X POST ${{ secrets.DEPLOY_WEBHOOK }} \\\n            -H \"Content-Type: application/json\" \\\n            -d '{\"image\": \"${{ env.REGISTRY }}/${{ env.IMAGE_NAME }}:${{ github.sha }}\", \"env\": \"production\"}'\n```",
    'example_ext': 'yaml',
}
NEW_CONTENT['deploy']['deploy/container-deploy/'] = {
    'title': '容器化部署规范',
    'overview': '容器化是现代化应用部署的标准方式。本规范定义了 Dockerfile 编写标准、镜像优化、安全扫描和容器运行时配置的最佳实践。',
    'principles': ['镜像精简：使用多阶段构建，最小化镜像体积', '单一职责：每个容器只运行一个进程', '不可变基础设施：容器视为一次性实体，不存储状态', '安全左移：在构建阶段就进行镜像安全扫描'],
    'rules': [
    ...
    'faqs': [
        ('Alpine vs Distroless vs Ubuntu？', 'Distroless（推荐）：只包含应用及其运行时依赖，无 shell/包管理器，攻击面最小。Alpine：体积小（5MB）、有包管理器、但 musl libc 可能导致兼容性问题。Ubuntu：最兼容、体积大、安全更新频繁。'),
        ('镜像体积应该多大？', '编译型语言（Go/Rust）目标 < 20MB，解释型语言（Python/Node）目标 < 200MB。超过 1GB 的镜像说明需要优化依赖管理或使用基础镜像。'),
    ],
    'checks': [
        'Dockerfile 使用多阶段构建',
        '基础镜像使用安全发行版',
...
NEW_CONTENT['deploy']['deploy/doc-4/'] = {
    'title': 'Kubernetes 部署管理',
    'overview': 'Kubernetes 是容器编排的事实标准。本规范涵盖了集群架构、Pod 配置、服务发现、Ingress 路由、配置管理和资源调度等生产级配置标准。',
    'principles': ['声明式管理：所有资源通过 YAML 声明，不手动操作', '自愈优先：利用 K8s 健康检查和自动重启机制', '渐进式发布：使用 Rolling Update / Blue-Green / Canary 策略', '资源隔离：通过 Namespace 和 ResourceQuota 实现租户隔离'],
    'rules': [
    ...
    'faqs': [
        ('Service 类型选型？', 'ClusterIP：集群内访问（微服务间通信）。NodePort：外部调试/测试。LoadBalancer：对外暴露服务（云厂商）。Ingress：HTTP/HTTPS 七层路由（对外统一入口）。'),
        ('Pod 健康检查配置多少合适？', 'liveness probe：检测应用是否存活（如进程死锁）。initialDelaySeconds: 30-60s，periodSeconds: 10-30s，failureThreshold: 3-5。readiness probe：检测应用是否可接受流量。initialDelaySeconds: 5-10s，periodSeconds: 5-10s。'),
    ],
    'checks': [
...
NEW_CONTENT['deploy']['deploy/iac/'] = {
    'title': '监控与告警规范',
    'overview': '监控告警是保障系统可靠性的核心手段。本规范定义了指标采集、日志管理、告警规则、通知路由和值班响应机制的标准化配置。',
    'principles': ['RED 方法：监控 Rate（速率）、Error（错误）、Duration（延迟）三个黄金指标', 'USE 方法：监控 Utilization（利用率）、Saturation（饱和度）、Error（错误）', '告警分优先级：P0 立即响应（5min）、P1 快速响应（30min）、P2 工作时间内处理', '避免告警风暴：聚合相似告警，防止重复通知轰炸'],
    'rules': [
    ...
    'faqs': [
        ('Prometheus vs Datadog vs Grafana Cloud？', 'Prometheus + Grafana（推荐）：开源、自托管、成本可控、生态丰富。Datadog：全托管、集成最全、但成本高。Grafana Cloud：托管的 Prometheus + Loki + Tempo 组合。'),
        ('告警阈值怎么设定？', '基于历史数据设定基线，基线值 × 1.5-2 倍作为告警阈值。例如系统正常 P95 延迟 200ms，设置告警阈值为 400ms 持续 5 分钟。避免使用固定值，使用动态基线更好。'),
    ],
    'checks': [
...
NEW_CONTENT['deploy']['deploy/doc-3/'] = {
    ...
    'faqs': [
        ('什么时候应该回滚而不是热修复？', '当问题影响用户核心功能 > 5%、或涉及数据安全时，应立即回滚。热修复只在影响面小（< 1% 用户）、修复时间短（< 30 分钟）的情况下使用。'),
        ('数据库回滚如何处理？', '数据库不执行传统「回滚」，而是执行前向降级脚本（down migration）。例如：v2 加了列，降级脚本是删除该列。关键原则：数据库只前向迁移，不后向回滚。'),
    ],
    'checks': [
        '发布有回滚方案',
        '回滚判定时间 < 15 分钟',
...
NEW_CONTENT['deploy']['deploy/doc-5/'] = {
    'title': '环境管理规范',
    'overview': '多环境管理是保障软件交付质量和安全的关键。本规范定义了开发、测试、预发布和生产环境的配置标准、访问控制、数据策略和环境一致性保障措施。',
    'principles': ['环境一致性：各环境尽量保持配置和基础设施一致，避免「在我机器上能跑」', '左移测试：在越早的环境发现并修复问题成本越低', '最小权限：每个环境的访问权限按需分配，生产环境严格管控', '可复现：环境可以通过 IaC（Infrastructure as Code）完全重建'],
    'rules': [
    ...
    'faqs': [
        ('环境越多越好吗？', '不是。推荐四环境模型：Dev（本地开发）→ Test（CI 自动测试）→ Staging（预发布验证）→ Production。每个环境有明确的入口和出口标准。环境多了反而增加维护成本。'),
        ('如何保证环境一致性？', '① IaC（所有环境使用同一套 Terraform 代码）② 容器化（运行时环境一致）③ 配置差异最小化（只有密钥、规模、日志级别的差异）④ 定期环境同步（Staging 数据定期从生产脱敏同步）。'),
    ],
    'checks': [
        '环境使用 IaC 管理',
        '预发布和生产环境一致',
...
# ===== AI DOMAIN =====
NEW_CONTENT['ai'] = {}
NEW_CONTENT['ai']['ai/rag/'] = {
    'title': 'RAG 系统设计',
    'overview': '检索增强生成（RAG）是将外部知识库与 LLM 结合的核心架构模式。本规范涵盖了文档分块策略、嵌入模型选择、向量数据库配置、检索优化和生成增强的最佳实践。',
    'principles': ['分块策略决定上限：文档分块的质量直接影响检索效果', '混合检索：关键词 + 向量 + 重排序三级检索提升精度', '上下文窗口管理：平衡检索数量与 Token 消耗', '可追溯：每个生成结果必须标注引用来源'],
    'rules': [
    ...
    'faqs': [
        ('Chunk Size 怎么设置？', '经验值：256-512 tokens（英文）、512-1024 tokens（中文）。根据文档类型调整：FAQ 类<100 tokens、技术文档 300-500 tokens、长文 500-800 tokens。关键原则：确保每个 Chunk 语义完整。'),
        ('RAG 检索不到正确文档怎么办？', '① 优化分块策略（重叠窗口 10-20%）② 增加元数据过滤（日期、类别、标签）③ 使用 Query Rewrite（将用户问题改写为更适合检索的形式）④ 增加 Hybrid Search 权重⑤ 使用 RAG Fusion（多查询合并结果）。'),
    ],
    'checks': [
        '分块策略文档化',
        '混合检索已实现',
...
NEW_CONTENT['ai']['ai/evaluation/'] = {
    'title': '模型评估规范',
    'overview': '模型评估是确保 AI 系统质量和可靠性的核心环节。本规范定义了评估数据集构建、评估指标选择、在线/离线评估流程和模型比较的标准方法。',
    'principles': ['任务匹配：评估指标与业务目标对齐，而非仅依赖通用指标', '多维评估：从准确性、安全性、鲁棒性、公平性多个维度评估', '自动化：评估流程自动化，每次模型更新自动触发', '人工兜底：自动化评估覆盖 80% 场景，边缘情况由人工评估'],
    'rules': [
    ...
    'faqs': [
        ('离线评估和在线评估的区别？', '离线评估（Offline）：使用标注数据集评估，快速、可复现、不影响用户。在线评估（Online）：通过 A/B 实验用真实用户流量评估，更准确但风险更大。推荐先离线评估通过后再做在线 A/B 实验。'),
        ('LLM 评估有哪些特殊挑战？', '① 答案多样性（同一个问题可以有多个正确答案）② 评估偏差（LLM-as-Judge 有位置偏好、权威偏好）③ 对抗输入（越狱提示、提示注入）④ 评估成本（人工评估耗时长、成本高）。'),
    ],
    'checks': [
        '评估数据集标准化',
        '自动化 + 人工评估已配置',
...
NEW_CONTENT['ai']['ai/ai-agent/'] = {
    'title': 'AI Agent 设计规范',
    'overview': 'AI Agent 是能够自主感知环境、制定计划并执行任务的智能系统。本规范定义了 Agent 的架构模式、工具调用规范、记忆管理和安全约束等设计标准。',
    'principles': ['目标驱动：Agent 行为由用户定义的目标驱动，非预设脚本', '工具即能力：Agent 的能力边界由可用工具集决定', '安全围栏：Agent 的自主性必须在明确的边界内', '可解释：Agent 的每一步决策都应有推理过程可追溯'],
    'rules': [
    ...
    'faqs': [
        ('Plan-and-Execute vs ReAct？', 'ReAct：推理和行动交错进行，适合逐步发现和解决的问题。Plan-and-Execute：先制定完整计划再执行，适合步骤明确的任务。复杂任务推荐 Plan-and-Execute 分解为子任务，每个子任务内部用 ReAct。'),
        ('Agent 记忆怎么管理？', '三层记忆：① 短期记忆（当前会话上下文，全部保留）② 长期记忆（跨会话关键信息，用向量数据库存储）③ 工作记忆（当前步骤的中间结果，步完成后清理）。使用 MemGPT/Letta 等框架管理记忆。'),
    ],
    'checks': [
        'Agent 使用 ReAct 模式',
        '工具调用有异常处理',
...
NEW_CONTENT['ai']['ai/ai/'] = {
    'title': 'AI 安全规范',
    'overview': 'AI 安全是确保 AI 系统行为可控、可靠且符合伦理的关键保障。本规范涵盖了提示注入防护、输出审核、模型后门检测、数据隐私保护和对抗攻击防御。',
    'principles': ['安全默认：安全机制默认开启，非可选功能', '纵深防御：输入过滤 + 模型内置安全 + 输出审核多层防护', '最小权限：AI 系统只获得完成任务所需的最小权限', '持续监控：安全防线需要持续评估和更新'],
    'rules': [
    ...
    'faqs': [
        ('如何防御提示注入攻击？', '多层防御：① 输入层：特殊指令模式检测 + 分割符隔离 ② 提示层：使用 Never 指令明确禁止 ③ 模型层：使用安全微调模型 ④ 输出层：检测脱轨输出。没有单层防御是完美的，必须多层结合。'),
        ('模型输出审核有哪些方法？', '① 规则引擎（正则 + 敏感词列表）② 安全分类器（独立的小模型判断内容安全性）③ LLM-as-Judge（使用另一个 LLM 审核输出）④ 人工抽检（高风险场景 100% 人工审核）。'),
    ],
    'checks': [
        '输入注入检测已实现',
        '输出安全审核已配置',
...
NEW_CONTENT['ai']['ai/mlops/'] = {
    'title': 'MLOps 规范',
    'overview': 'MLOps 将 DevOps 实践应用于机器学习生命周期管理，实现模型开发、训练、部署和监控的自动化。本规范涵盖了实验跟踪、模型注册、Pipeline 编排和模型治理。',
    'principles': ['可复现：每次实验的所有参数、数据和代码版本都可追溯', '自动化：模型训练、评估和部署流程全自动化', '模型治理：模型版本、审批和发布有完整的生命周期管理', '持续监控：模型上线后持续监控性能退化（Concept Drift / Data Drift）'],
    'rules': [
    ...
    'faqs': [
        ('MLflow vs Kubeflow vs Airflow？', 'MLflow：实验跟踪 + 模型注册，轻量级，适合团队起步。Kubeflow：端到端 ML Pipeline 平台，K8s 原生，适合大规模部署。Airflow：通用工作流编排，适合复杂的 ETL + 训练混合流程。通常组合使用：MLflow + 任意的 Pipeline 工具。'),
        ('模型漂移如何检测？', 'Data Drift：使用统计检验（KS 检验/PSI）比较当前数据和训练数据的分布差异。Concept Drift：监控模型预测分布和实际标签分布的变化。阈值设定：PSI > 0.2 或 KS p-value < 0.05 触发告警。'),
    ],
    'checks': [
        '实验跟踪已启用',
        '代码/数据/配置版本化',
...
NEW_CONTENT['ai']['ai/training-pipeline/'] = {
    'title': '模型训练流水线',
    'overview': '模型训练流水线是 MLOps 的核心组成部分，实现从数据处理到模型产出的自动化。本规范涵盖了数据准备、训练配置、分布式训练、超参搜索和训练监控。',
    'principles': ['数据质量：训练数据的质量决定了模型性能的上限', '可复现：相同代码 + 相同数据 + 相同配置 = 相同模型', '增量迭代：在已有模型基础上增量训练，而非每次从头开始', '资源效率：合理利用 GPU 资源，避免闲置和浪费'],
    'rules': [
    ...
    'faqs': [
        ('分布式训练的并行策略？', 'Data Parallel（数据并行）：每个 GPU 处理不同 batch，梯度同步。Model Parallel（模型并行）：不同 GPU 处理模型不同层。Pipeline Parallel（流水线并行）：将模型按层分组到不同 GPU。3D Parallel 是三种组合。'),
        ('GPU 利用率不高怎么办？', '① 增大 batch size ② 使用 DataLoader 的 num_workers 预加载 ③ 使用梯度累积 ④ 检查是否存在 CPU 瓶颈（数据加载成为瓶颈）⑤ 使用 NVIDIA DALI 加速数据加载。'),
    ],
    'checks': [
        '数据集有质量门禁',
        '训练/验证/测试集划分正确',
...
NEW_CONTENT['ai']['ai/model-serving/'] = {
    'title': '模型服务规范',
    'overview': '模型服务是 AI 系统面向用户提供推理能力的接口层。本规范涵盖了模型部署架构、推理优化、负载管理、版本路由和监控告警的标准化配置。',
    'principles': ['低延迟优先：在线推理应在 500ms 内完成', '高吞吐优先：离线批量推理最大化 GPU 利用率', '弹性伸缩：根据负载自动伸缩推理实例数量', '优雅降级：模型不可用时提供降级方案（缓存/规则引擎/默认值）'],
    'rules': [
    ...
    'faqs': [
        ('Triton vs TorchServe vs BentoML？', 'Triton Inference Server（推荐）：NVIDIA 出品、支持多框架、动态批处理、GPU 优化最好。TorchServe：PyTorch 原生、易于使用。BentoML：Python 原生、生态系统好、支持自定义框架。'),
        ('模型推理优化方法？', '① 模型量化（FP16/INT8）② 模型剪枝 ③ ONNX Runtime/TensorRT 加速 ④ KV Cache 优化（LLM）⑤ 请求批处理（增大吞吐）⑥ 推理结果缓存。通常组合使用可提升 2-5 倍性能。'),
    ],
    'checks': [
        '服务有健康检查和预热',
        '推理超时已配置',
...
NEW_CONTENT['ai']['ai/ai-ethics/'] = {
    'title': 'AI 伦理规范',
    'overview': 'AI 伦理规范确保 AI 系统的开发和使用符合道德标准和社会责任。本规范涵盖了公平性、透明度、隐私保护、问责制和人文价值等核心原则的落地实践。',
    'principles': ['公平公正：AI 系统不应因种族、性别、年龄等特征产生歧视', '透明可解释：AI 的决策过程应能被理解和解释', '隐私保护：AI 系统的数据处理需尊重用户隐私权和数据自主权', '人文主导：AI 是辅助人类决策的工具，不能完全取代人类判断'],
    'rules': [
    ...
    'faqs': [
        ('如何检测 AI 偏见？', '在不同子群体上分别评估模型准确率。如果某个群体的准确率显著低于平均，说明存在偏见。使用公平性指标：Demographic Parity、Equal Opportunity、Equalized Odds。'),
        ('AI 的「黑盒」问题怎么解决？', '使用可解释 AI 工具：SHAP（特征重要性）、LIME（局部解释）、Grad-CAM（视觉解释）。对于高风险决策，使用可解释性强的模型（如决策树、逻辑回归）作为备选。'),
    ],
    'checks': [
        '公平性审计已完成',
        '高风险场景有人工复核',
...
def merge_and_write():
    """Merge NEW_CONTENT into existing CONTENT and write back to knowledge_content.py"""
    merged = dict(existing)
    
    for domain, topics in NEW_CONTENT.items():
        if domain not in merged:
            merged[domain] = {}
        for topic_key, content in topics.items():
            merged[domain][topic_key] = content
    
    # Generate the Python file content
    lines = ['import yaml, os', 'from pathlib import Path', '', '# Auto-generated knowledge content database', '# Edit this file to customize content for specific knowledge dirs', '', 'CONTENT = {']
    
    for domain in sorted(merged.keys()):
        lines.append(f"    {domain!r}: {{")
        for topic_key in sorted(merged[domain].keys()):
            c = merged[domain][topic_key]
            lines.append(f"        {topic_key!r}: {{")
            lines.append(f"            'title': {c['title']!r},")
            lines.append(f"            'overview': {c['overview']!r},")
            lines.append(f"            'principles': {c['principles']!r},")
            lines.append(f"            'rules': {c['rules']!r},")
            lines.append(f"            'faqs': {c['faqs']!r},")
            lines.append(f"            'checks': {c['checks']!r},")
            lines.append(f"            'prompt_sp': {c['prompt_sp']!r},")
            lines.append(f"            'prompt_up': {c['prompt_up']!r},")
            lines.append(f"            'example_text': {c['example_text']!r},")
            lines.append(f"            'example_ext': {c['example_ext']!r},")
            lines.append('        },')
        lines.append('    },')
    ...
    output = '\n'.join(lines)
    target = BASE / 'scripts' / 'knowledge_content.py'
    target.write_text(output, encoding='utf-8')
    
    # Stats
    total_topics = sum(len(topics) for topics in merged.values())
    total_domains = len(merged)
    print(f"Written {total_topics} topics across {total_domains} domains to {target}")
    print(f"Domains: {', '.join(sorted(merged.keys()))}")
    print(f"New content added: {sum(len(topics) for topics in NEW_CONTENT.values())} topics")
    
    # Verify
    import knowledge_content as kc2
    reload_topics = sum(len(t) for t in kc2.CONTENT.values())
    print(f"Verification: {reload_topics} topics loaded successfully")
